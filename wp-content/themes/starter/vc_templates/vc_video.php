<?php
if ( ! defined( 'ABSPATH' ) ) {
	die( '-1' );
}

/**
 * Shortcode attributes
 * @var $atts
 * @var $title
 * @var $link
 * @var $el_class
 * @var $el_id
 * @var $css
 * @var $css_animation
 * @var $el_width
 * @var $el_aspect
 * @var $align
 * Shortcode class
 * @var WPBakeryShortCode_Vc_Video $this
 */
$title = $link = $el_class = $el_id = $css = $css_animation = $el_width = $el_aspect = $align = '';
$atts = vc_map_get_attributes( $this->getShortcode(), $atts );
extract( $atts );

if ( '' === $link ) {
	return null;
}
$el_class = $this->getExtraClass( $el_class ) . $this->getCSSAnimation( $css_animation );

$video_w = 500;
$video_h = $video_w / 1.61; // 1.61 golden ratio
/** @var WP_Embed $wp_embed */
global $wp_embed;
$embed = '';
if ( is_object( $wp_embed ) ) {
	$embed = $wp_embed->run_shortcode( '[embed width="' . $video_w . '"' . $video_h . ']' . $link . '[/embed]' );
}
$el_classes = array(
	'gm_video',
	'gm_content-element',
	$el_class,
	vc_shortcode_custom_css_class( $css, ' ' ),
	'gm_video-aspect-ratio-' . esc_attr( $el_aspect ),
	'gm_video-el-width-' . esc_attr( $el_width ),
	'gm_video-align-' . esc_attr( $align )
);


if ( ! empty( $atts['content_margin_top'] ) ) {
	$el_classes[] = ' gm_content-element--margin-top-' . $atts['content_margin_top'];
}

if ( ! empty( $atts['content_margin_bottom'] ) ) {
	$$el_classes[] = ' gm_content-element--margin-bottom-' . $atts['content_margin_bottom'];
}


$css_class = implode( ' ', $el_classes );
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $css_class, $this->getShortcode(), $atts );

$wrapper_attributes = array();
if ( ! empty( $el_id ) ) {
	$wrapper_attributes[] = 'id="' . esc_attr( $el_id ) . '"';
}

$output = '
	<div class="' . esc_attr( $css_class ) . '" ' . implode( ' ', $wrapper_attributes ) . '>
		<div class="gm_video_wrapper">' . $embed . '</div>
	</div>
';

return $output;
