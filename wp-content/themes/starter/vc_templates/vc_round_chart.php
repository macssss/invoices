...
$canvas_html = '<canvas class="gm_round-chart-canvas" width="1" height="1"></canvas>';
$legend_html = '';
if ( $legend ) {
	foreach ( $data as $v ) {
	...
	$legend_html = '<ul class="gm_chart-legend">' . $legend_html . '</ul>';
}
if ( ! empty( $el_id ) ) {
...
$output = '';
$output .= '<div class="' . esc_attr( $css_class ) . '" ' . implode( ' ', $options ) . '>';

if ( $legend ) {
	$output .= '<div class="gm_chart-inner uk-grid uk-grid-small uk-flex-middle" data-uk-grid-margin>';
		$output .= '<div class="uk-width-medium-6-10">' . $canvas_html . '</div>';
		$output .= '<div class="uk-width-medium-4-10">' . $legend_html . '</div>';
	$output .= '</div>';
}

else {
	$output .= '<div class="gm_chart-inner">' . $canvas_html . '</div>';
}
