...
class CommentApproved {
	...
	public function settings() {
		...
		if ( isset( $_POST['comment_approved_settings'] ) ) {
			...
			$from = ( $_POST['comment_approved_from'] );
			$reply_to = ( $_POST['comment_approved_reply_to'] );

			update_option( 'comment_approved_message', $message );
			update_option( 'comment_approved_subject', $subject );
			update_option( 'comment_approved_from', $from );
			update_option( 'comment_approved_reply_to', $reply_to );
		...
		$from = get_option( 'comment_approved_from' );
		$reply_to = get_option( 'comment_approved_reply_to' );
		...
		<div class="wrap">
			...
			<form method="post">
				...
				<table class="form-table" id="wp-comment-approved-settings">
					...
					<tr class="default-row">
						<th><label><?php esc_html_e( 'From', 'comment-approved' ); ?></label></th>
						<td>
							<input type="text" name="comment_approved_from" class="large-text" value="<?php echo esc_attr( $from ); ?>" />
						</td>
					</tr>
					<tr class="default-row">
						<th><label><?php esc_html_e( 'Reply-To', 'comment-approved' ); ?></label></th>
						<td>
							<input type="text" name="comment_approved_reply_to" class="large-text" value="<?php echo esc_attr( $reply_to ); ?>" />
						</td>
					</tr>
					<tr class="default-row">
	...
	public function approve_comment_callback( $comment ) {
		...
		wp_mail( $comment->comment_author_email, $subject, $notification, $headers );
	...
	public function approve_comment_option( $post_id ) {
		...
		printf(
			'<span class="comment-form-notify-me wpcf7-checkbox column">
				<span class="wpcf7-list-item">
					<label>
						<input type="checkbox" %s name="comment-approved_notify-me" value="1" />
						<span class="wpcf7-list-item-label">%s</span>
					</label>
				</span>
			</span>',
