...
function crp_author( $args, $result ) {
	...
	if ( ! empty( $author_name ) ) {
		$crp_author = '<div class="gm_item__author"> ' . __( ' by ', 'contextual-related-posts' ) . '<a href="' . $author_link . '">' . $author_name . '</a></div> ';
	} else {
		$crp_author = '';
	}
...
function crp_list_link( $args, $result ) {
	...
	if ( 'after' === $args['post_thumb_op'] ) {
		$output .= '<h4 class="gm_item__title uk-h6">';
			$output .= '<a href="' . $link . '" ' . $link_attributes . '>' . $title . '</a>';
		$output .= '</h4>'; // Add title when required by settings.
	}

	if ( 'inline' === $args['post_thumb_op'] || 'after' === $args['post_thumb_op'] || 'thumbs_only' === $args['post_thumb_op'] ) {
		
		$output .= '<div class="gm_item__image">';
