...
function get_crp( $args = array() ) {
	...
	$widget_class    = $args['is_widget'] ? 'crp_related_widget ' : 'gm_related-posts';
	...
	if ( $results ) {
		...
		foreach ( $results as $result ) {
			...
			if ( $args['show_date'] ) {
				$output .= '<div class="gm_item__date">' . mysql2date( get_option( 'date_format', 'd/m/y' ), $result->post_date ) . '</div> ';
			}

			if ( $args['show_excerpt'] ) {
				$output .= '<span class="gm_item__excerpt">' . crp_excerpt( $result->ID, $args['excerpt_length'] ) . '</div>';
			}
	...
	if ( false === ( strpos( $output, $args['before_list_item'] ) ) ) {
		$output  = '<div id="gm_related-posts">';
		$output .= ( $args['blank_output'] ) ? ' ' : '<p>' . $args['blank_output_text'] . '</p>';
	}
